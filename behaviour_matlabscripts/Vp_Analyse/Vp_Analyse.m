...
et = 2; % Analyse repeats as one experiment (1) or treat repeats seperatly (2) 
ct{1} = [2 3]; % days of interest % generally 2 3
ct{2} = [2 3]; % nights of interest % generally 2 3
...
% clear pathname filename geno f; % FK commented out
clear geno f; % FK I want to use filename to export stuff
...
% FK 04/03/19: added if loop (original is above)
% problem when no offset: find(...) gets both columns
if size(lb_sec, 2) == 2 % if 2 boxes
    
    if lb_sec(:,1) == lb_sec(:,2)
        disp ('No offset');
        offset = [0 0];
        c = 2; % random, as both cols are the same
    else
        [~,c] = find(lb_sec == max(lb_sec(2,:)));
        % Find the column (c) with the longest starting window (in seconds) 
        for e = 1:size(lb_sec,2) % For each experiment
...
elseif size(lb_sec, 2) == 1
    disp ('Single box ran >> no offset')
    offset = [0 0];
    c = 1;
end
...
% FK 12/03/2019
% indexing issue in the specific case where
% group1 > group2 in exp1 & group2 > group1 in exp2
% dirty fix is to change one of the group size to size of
% parameter_comparisons
tmp = size(parameter_comparisons{1});

group_sizes_tmp = group_sizes;

if ismember(tmp(1), group_sizes)
    disp('No need to create dummy group_sizes for ANOVA');
else
    disp('Creating a dummy value in group_sizes for ANOVA');
    group_sizes_tmp(1) = tmp(1);
end
% below: group_sizes is replaced group_sizes_tmp
% FK done
...
for g = 1:max(group_tags) % For each group 
    anova_group = [anova_group  ones(max(group_sizes_tmp),1)*g]; % Allocate tag 
...
for t = time_window(1):time_window(2) % For each time window 
    anova_time = [anova_time ones(max(group_sizes_tmp),max(group_tags))*mod(t,2)]; 
...
%% Export stats
% FK added this part 11/04/19

% export ANOVA
savename_stats = strcat(filename{1}(1:9));
csvwrite (strcat (pathname, '/', savename_stats, '_anova.csv'), twa.p);

% export KruskalWallis
csvwrite (strcat (pathname, '/', savename_stats, '_kwtime_h.csv'), kw.time_h);
csvwrite (strcat (pathname, '/', savename_stats, '_kwtime_p.csv'), kw.time_p);

csvwrite (strcat (pathname, '/', savename_stats, '_kwgroup_h.csv'), horzcat(kw.group_h{:}));
csvwrite (strcat (pathname, '/', savename_stats, '_kwgroup_p.csv'), horzcat(kw.group_p{:}));

%FK FK FK below to export trace of a specific parameter
% export active bout length trace
% it is parameter 1
% csvwrite (strcat (pathname, '/', savename_stats, '_activeboutlength.csv'), parameter_time_sec_smooth{1,1});
% csvwrite (strcat (pathname, '/', savename_stats, '_activeboutlength_t.csv'), parameter_time_sec_smooth_y{1,1});

% export lbsec
csvwrite (strcat (pathname, '/', savename_stats, '_lbsec.csv'), lb_sec);

% export delta_px_sq
csvwrite (strcat (pathname, '/', savename_stats, '_deltapxsecsmooth2.csv'), delta_px_sq_sec_smooth)