...
% Data comes from 96 regions of interest (ROIS) %140619 FK 192 >> 96
...
days = [1 2 3 4]; % number of days % typically 1 2 3 4
nights = [1 2 3]; % number of nights % typically 1 2 3
...
for f = 1:size(folder_open,1) % For each excel file 
    sheet_names{f} = folder_open(f).name; % Take its name 
...
for f = O' % For each Excel file
    ...
    if f == O(1) % For the first file Skip the first blank lines
        raw_text = textscan(fid,... % Read in the data
            '%*f32 %f32 %*f32 %f32 %*1s %s %f32','headerlines',96 + 1); %140619 FK 192 >> 96
    ...
    found = find(fish_id_order_check ~= 1 & fish_id_order_check ~= -95 &... %140619 FK 191 >> 95
    ...
    while isempty(found) == 0 % While there are dropped frames 
        ...
        % Cut backwards
        found_clip_b = [95 1]; %140619 FK 191 >> 95
        if found - found_clip_b(1) < 1
            found_clip_b = [found + 1 found + 1]; 
        else
            while sum(fish_id_order_check(found - found_clip_b(1):found - found_clip_b(2))) ~= 95 %140619 FK 191 >> 95
        ...
        % Cut forwards
        found_clip_f = [1 95]; %140619 FK 191 >> 95
        if found + found_clip_f(2) > length(fish_id_order_check)
            ...
            while sum(fish_id_order_check(found + found_clip_f(1):found + found_clip_f(2))) ~= 95 %140619 FK 191 >> 95
...
check(:,2) = fish_id(scrap == 1); % Values (should all be 96) %140619 FK 192 >> 96

if isempty(find(check(:,2) ~= 96)) == 0 % If there are errors %140619 FK 192 >> 96
    err = find(check(:,2) ~= 96); % define them %140619 FK 192 >> 96
    ...
    for e = 1:size(err,1) % for each error
        ...
        if found - found_clip_b(1) < 1
            ...
            while sum(fish_id_diff(found - found_clip_b(1):found - found_clip_b(2))) ~= 95 %140619 FK 191 >> 95
                found_clip_b = found_clip_b + 1; % Slide window
...
if isempty(find(check(:,2) ~= 1)) == 0 % If there are errors
    ...
    for e = 1:size(err,1) % for each error
        ...
        if found + found_clip_f(2) > length(fish_id_diff)
            ...
            while sum(fish_id_diff(found + found_clip_f(1):found + found_clip_f(2))) ~= 95 %140619 FK 191 >> 95
                found_clip_f = found_clip_f + 1; % Slide window
...
%%% FK FK FK (if playing with replays, ZebraLab may generate a .xls file
%%% that is not read correctly
if isnan (start_time) == 1 
    error('Error while reading the DATA file (start time is NaN)')
end
%%% FK FK FK
...
if isempty(top_up) == 0 % if fish h20 was topped up
    ...
    fps = round(1/((nanmean(diff(time_sq_max)))*(60*60))); % Calculate frame rate % FK 11/12/2018, was round(1/((nanmean(diff(time_sq_max)))*(60*60)),1)
    % round (X, 1) allows one decimal. Happens that not an integer, which
    % can cause problem later when used as index for arrays.
...
fps = round(1/((nanmean(diff(time_sq_max)))*(60*60))); % FK 11/12/2018, same as line 538
