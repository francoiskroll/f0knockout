# TRACE BY GENOTYPE
# may want to crop same way Marcus' plot (ie. start at 1st full day)

# v2: generalised for > 2 genotypes
# v3: guesses the paths/names of genotype & lbsec files
# v4: smoothing (like parameterTrace.R)
# v5: plots from delta_px_sq_sec_smooth2 (from VpAnalyse, should be the same though) 
  # & plotting for A4 & exports in pdf (solves transparency issue)
...
# import ------------------------------------------------------------------
file <- '~/.../190813_06_deltapxsecsmooth.csv'
...
genofile <- '190819_06genotype.txt'
...
mycols <- c('#697a87', '#c23712') # colours so fits fingerprint figure
ribboncols <- c('#ADB4BA', '#D69D8C') # manually setting ribbon colours so cover the grid
...
# pdf too heavy; export in high quality png should help; but takes ages
tic('png export')
ggsave (filename='f0_scn1labtrace_06_small.png', plot=dpxtrace, width=92, height=70, units='mm', bg='transparent', dpi=1000)
toc()
